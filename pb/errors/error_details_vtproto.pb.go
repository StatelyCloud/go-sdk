...
func (m *StatelyErrorDetails) CloneVT() *StatelyErrorDetails {
	if m == nil {
		return (*StatelyErrorDetails)(nil)
	}
	r := new(StatelyErrorDetails)
...
func (this *StatelyErrorDetails) EqualVT(that *StatelyErrorDetails) bool {
	...
	if this.StatelyCode != that.StatelyCode {
		return false
	}
...
func (this *StatelyErrorDetails) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*StatelyErrorDetails)
	if !ok {
		return false
	}
...
func (m *StatelyErrorDetails) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.UpstreamCause) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.UpstreamCause)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Message) > 0 {
	...
	if len(m.StatelyCode) > 0 {
		i -= len(m.StatelyCode)
		copy(dAtA[i:], m.StatelyCode)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.StatelyCode)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *StatelyErrorDetails) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: StatelyErrorDetails: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: StatelyErrorDetails: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field StatelyCode", wireType)
			}
			...
			m.StatelyCode = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field UpstreamCause", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
