...
func (m *ContinueListRequest) CloneVT() *ContinueListRequest {
	if m == nil {
		return (*ContinueListRequest)(nil)
	}
...
func (this *ContinueListRequest) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*ContinueListRequest)
	if !ok {
		return false
	}
...
func (m *ContinueListRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: ContinueListRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ContinueListRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
